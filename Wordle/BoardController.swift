...
class BoardController: NSObject,
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let themeSettings = settings[kWordThemeKey] as? String,
      let theme = WordTheme(rawValue: themeSettings) {
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let alienSettings = settings[kIsAlienWordleKey] as? Bool {
          isAlienWordle = alienSettings
      }
    // START YOUR CODE HERE
  }
}
